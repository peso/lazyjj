...
use core::fmt;
use ratatui::crossterm::{self, event::{Event as TermEvent, KeyCode, KeyModifiers}};
...
impl<'a> App<'a> {
    ...
    pub fn launch_input_channel(&mut self) {
        let jj_folder = PathBuf::from(&self.env.root.clone()).join(".jj");
    ...
    pub fn input(&mut self, event: AppEvent, commander: &mut Commander) -> Result<bool> {
        let AppEvent::UserInput(event) = event
        else {
